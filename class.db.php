...
class DB
{
     ...
     * @access public
     * @param1 string query
     * @param2 boolean return_single_row (Default:true)
     * @return array
     *
     */
    public function get_results( $query, $return_single_row = true )
    {
        ...
        else
        {
			...
			# Check single array or not & return specific values
			if(count($row) == 1 && $return_single_row){
				return $row[0];
			} else {
				return $row;
			}
	...
	/**
     * Select specific value from the selected table
     *
     * @access public
     * @param string table name
     * @param string return field name
     * @param string check field name
     * @param string check field value
     * @return string return field value
     *
     */
	public function select($table, $return_field, $check_field, $check_value) {
		$sql	= "SELECT " . $return_field . " FROM " . $table . " WHERE " . $check_field . " = '" . $check_value . "'";
		list($return_val)	= $this->get_row($sql);
		return $return_val;	
	}
